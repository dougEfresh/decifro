...
import { compiledInstructionToInstruction, flattenTransactionResponse, parseTransactionAccounts } from "./helpers";
import {
...
export class SolanaParser {
	...
	constructor(programInfos: ProgramInfoType[], parsers?: InstructionParserInfo[]) {
		...
		if (!parsers) {
			...
			// append parsers if parser not exist yet
	...
	async parseTransactionDump(connection: Connection, txDump: string | Buffer): Promise<ParsedInstruction<Idl, string>[]> {
		...
		if (vtx.version !== "legacy") {
			...
			if (accountsToFetch.length > 0) {
				...
				const altAccounts = fetched.filter((f) => f !== null && f.data.length > 0).map((f) => AddressLookupTableAccount.deserialize(f.data));
