...
describe("parse stake", () => {
	it("should parse StakeProgram.createAccount instruction", async () => {
		...
		assert.equal(result.parsedInstructions.length, 2);
		const createAccount = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "initialize">;
		const auth = result.parsedInstructions[1] as ParsedIdlInstruction<StakeLayout, "authorize">;
	...
	it("should parse StakeProgram.withdraw instruction", async () => {
		...
		const result = await parser.parseTransactionDump(connection, tx);
		assert.isDefined(result);
		assert.equal(result.parsedInstructions.length, 1);
		const withdraw = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "withdraw">;
	...
	it("should parse StakeProgram.delegate instruction", async () => {
		...
		assert.isDefined(result);
		assert.equal(result.parsedInstructions.length, 1);
		const delegate = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "delegate">;
	...
	it("should parse StakeProgram.spliit instruction", async () => {
		...
		const ca = result.parsedInstructions[0] as ParsedIdlInstruction<SystemProgramIdl, "createAccount">;
		assert.equal(ca.name, "createAccount");
		assert.equal(ca.programId.toBase58(), SystemProgram.programId.toBase58());
		const split = result.parsedInstructions[1] as ParsedIdlInstruction<StakeLayout, "split">;
	...
	it("should parse StakeProgram.authorized instruction", async () => {
		...
		assert.equal(result.parsedInstructions.length, 1);
		//console.log(JSON.stringify(result, undefined, 2));
		const auth = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "authorize">;
	...
	it("should parse StakeProgram.authorizedWithSeed instruction", async () => {
		...
		assert.equal(result.parsedInstructions.length, 1);
		//console.log(JSON.stringify(result, undefined, 2));
		const auth = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "authorizeWithSeed">;
	...
	it("should parse StakeProgram.deactivate instruction", async () => {
		...
		assert.equal(result.parsedInstructions.length, 1);
		//console.log(JSON.stringify(result, undefined, 2));
		const auth = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "deactivate">;
	...
	it("should parse StakeProgram.merge instruction", async () => {
		...
		assert.equal(result.parsedInstructions.length, 1);
		//console.log(JSON.stringify(result, undefined, 2));
		const auth = result.parsedInstructions[0] as ParsedIdlInstruction<StakeLayout, "merge">;
	...
	it("should parse StakeProgram.createAccountWithSeed instruction", async () => {
		...
		assert.isDefined(result);
		assert.equal(result.parsedInstructions.length, 2);
		const createAccount = result.parsedInstructions[0] as ParsedIdlInstruction<SystemProgramIdl, "createAccountWithSeed">;
		const auth = result.parsedInstructions[1] as ParsedIdlInstruction<StakeLayout, "authorize">;
